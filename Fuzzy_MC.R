#' @title Multilevel clustering - Fuzzy and evidential
...
Fuzzy_MC <- function(X, levelMax, choice="kmean", criteria="CardSil",masscrit="bel",
  ...
  while(!stop){
    ...
    sapply(clusterToCut, FUN = function(x){
      ...
      if(nrow(Xprime) > minPoint){
        ...
        kClusters <- Kbrut(Xprime,Kmax=7,nstart=20)
        ...
        else if(choice == "cmean")
        {
          ...
          if(criteria == "FuzzySilhouette")
          {
            ...
            silLevel <- sapply(uniqueCluster, FUN = function(x){
              FSIL(Xprime,cluster,x,clus$membership)
            })
        ...
        {
          clust <- ecm(Xprime, kClusters, type="full", init="rand", beta=beta, delta=delta,alpha=alpha,ntrials=nstart,disp=FALSE)
          groups <- clust$y.bel
          ...
          if(criteria == "EvidentialSilhouette")
          {
            ...
            silLevel <- sapply(uniqueCluster, FUN = function(x){
              ((length(clust$lower.approx[[x]])/length(clust$upper.approx[[x]]))^SIL.F(Xprime[cluster == x,], clust$mass.n[cluster == x,]))
            })
          ...
          if(criteria == "FuzzySilhouette")
          {
            ...
            silLevel <- sapply(uniqueCluster, FUN = function(x){
              FSIL(Xprime,cluster,x,clust$bel.n)
            })
          ...
          if(criteria == "mass")
          {
            ...
            {
              silLevel <- sapply(uniqueCluster, FUN = function(x){
                sortedpl <- sort(clust$pl.n[clust$lower.approx[[x]],x])
                sortedpllow <- sortedpl[1:length(sortedpl)/3]
                
                sum(sortedpllow)/length(sortedpllow)
              })
            }
            if(masscrit == "lowbel100")
            {
    ...
    {####
      cardinal <- sapply(uniqueCluster, FUN = function(x){
        sum(cluster::silhouette(cluster,dist(X))[,"sil_width"][which(cluster == x)] < 0)
      })
    ...
    {####
      silLevel <- sapply(uniqueCluster, FUN = function(x){
        mean(cluster::silhouette(cluster,dist(X))[,"sil_width"][which(cluster == x)])
      })
