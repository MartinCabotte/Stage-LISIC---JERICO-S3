#' @title Analyse des critères avec des algorithmes non itératif
#' @author Martin CABOTTE
#' @description 
#' @param data the dataset.
#' @param trueLabels true labels of the dataset.
#' @param trueK If user wants to perform analysis using the trueK value.
...
analyseCritereNonIteratif <- function(data,trueLabels,spectral=FALSE,trueK=FALSE,
                           iteration,
                           algorithme,
                           nstart=10,
                           filenameLabel,filenameResults,
                           beta=2, delta=10, alpha=1,
                           verbose=FALSE, vois=3,
                           flagDiagZero=FALSE,Kmax=20,tolerence = 0.99,threshold = 0.9)
{
  #Parameters
  ...
  for(i in 1:iteration)
  {
    ...
    if(spectral == TRUE)
    {
      ...
      if(verbose){print(eigenValues[1:10])}
    ...
    {
      dataSpec <- data
      kClusters <- Kbrut(data,Kmax=Kmax,nstart=30,verbose=FALSE)
    }
    
    if(trueK == TRUE)
    {
      kClusters <- max(trueLabels)
    }
    
    
    if(algorithme == "kmean"){result <- kmeans(dataSpec,kClusters,nstart=nstart,iter.max = 100)$cluster}
    if(algorithme == "cmean")
    {
    ...
    if(i == 1)
    {
      write.csv(result,filenameLabel,row.names = FALSE)
    }
    else
    {
      previousLabels <- read.csv(filenameLabel)
      unlink(filenameLabel)
      Labels <- cbind(previousLabels, result)
      write.csv(Labels,filenameLabel,row.names = FALSE)
    }
    
    clus = length(unique(result))
    ...
    totalLabel <- unique(as.factor(result))
    ...
    {
      label[j] <- array$label[which(result[j] == array$algo)]
    }
    ...
    if(max(unique(label)) != 1)
    {
      ...
      sil <- cluster::silhouette(as.numeric(factor(result)),dist(data))
      recap <- summary(sil)
      ...
      currentAri <- adj.rand.index(trueLabels,as.numeric(factor(result)))
      ari = ari + currentAri
      ...
      currentVmes <- v_measure(trueLabels, as.numeric(factor(result)), beta = 1)
      ...
      assign <- assignVote(trueLabels,as.integer(factor(result)))
      ...
      currentOverlapvar <- Overlap(trueLabels,as.integer(factor(result)))
